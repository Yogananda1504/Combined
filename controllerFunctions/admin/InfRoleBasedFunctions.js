...
import { Network_logger } from "../../utils/admin/logger.js";
import { Electricity_logger } from "../../utils/admin/logger.js";
...
export const DepartmentDataController = async (req, res) => {
	try {
		...
		getLogger(req.role).error(
			`Error fetching department data: ${error.message}`
		);
...
export const DepartmentComplaintStatusController = async (req, res) => {
	try {
		...
		getLogger(role).info(
			`${complainType} admin ${status} complaint ${id} at ${new Date().toISOString()}`
		);
		return res.json({ success: true, complaint });
	} catch (error) {
		getLogger(req.role).error(
			`Error updating complaint status: ${error.message}`
		);
		return res
			.status(500)
...
export const departmentRemarkController = async (req, res) => {
	try {
		...
		const queryFilter =
			role === "admin" ? { _id: id } : { _id: id, complainType };
		...
		getLogger(role).info(
			`${
				role === "admin" ? "Admin" : complainType
			} updated remarks for complaint ${id} at ${new Date().toISOString()}`
		);

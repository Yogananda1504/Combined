...
import { Network_logger as logger } from "../../utils/admin/logger.js";
...
export const NetworkComplaintStatusController = async (req, res) => {
	try {
		...
		logger.info(
			`Admin ${status} Network complaint ${id} at ${new Date().toISOString()}`
		);
...
export const NetworkStatsController = async (req, res) => {
	try {
		...
		return res
			.status(500)
			.json({ success: false, message: "Error in fetching network stats" });
	}
};
...
export const networkRemarkController = async (req, res) => {
	try {
		...
		logger.info(
			`Admin updated remarks for Network complaint ${id} at ${new Date().toISOString()}`
		);
		res.json({ success: true, complaint });
	} catch (error) {
		logger.error(
			`Error updating remarks for Network complaint: ${error.message}`
		);
		res
			.status(500)
